...
def square_mask(shape):
    mask = torch.tensor((244, 244), dtype=torch.float)
    return mask
...
def apply_patch(data, patch, transforms):
    ...
    mask = square_mask(data.shape[1:]).cuda()
    mask_batch = mask.repeat(bsz, 1, 1, 1)
    patch_batch = patch.repeat(bsz, 1, 1, 1)


    coeffs = affine_coeffs(*transforms)
    grid = F.affine_grid(coeffs, data.size(), align_corners=True).cuda()

    mask_t = F.grid_sample(mask_batch, grid, align_corners=True)
    patch_t = F.grid_sample(patch_batch, grid, align_corners=True)
...
def tensor_to_pil(image, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225), clip=False):
    ...
    if clip:
        out = out * square_mask(out.size()).cuda()
